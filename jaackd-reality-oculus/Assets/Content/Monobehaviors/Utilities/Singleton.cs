...
#if UNITY_EDITOR
          Utilities.PrintWarningIfEditor("[Singleton] Instance '{0}' already destroyed on application quit. Won't create again - returning null.", typeof(T));
#endif
          return null;
        }
        lock (_lock) {
          if (!instantiated) {
            Object[] objects;
            if (FindInactive) { objects = Resources.FindObjectsOfTypeAll(typeof(T)); } else { objects = FindObjectsOfType(typeof(T)); }
            if (objects == null || objects.Length < 1) {
              GameObject singleton = new GameObject();
              singleton.name = string.Format("{0} [Singleton]", typeof(T));
              Instance = singleton.AddComponent<T>();

              Utilities.PrintWarningIfEditor("[Singleton] An Instance of '{0}' is needed in the scene, so '{1}' was created{2}", typeof(T), singleton.name, Persist ? " with DontDestoryOnLoad." : ".");

            } else if (objects.Length >= 1) {
              Instance = objects[0] as T;
              if (objects.Length > 1) {

                Utilities.PrintWarningIfEditor("[Singleton] {0} instances of '{1}'!", objects.Length, typeof(T));

                if (DestroyOthers) {
                  for (int i = 1; i < objects.Length; i++) {

                    Utilities.PrintWarningIfEditor("[Singleton] Deleting extra '{0}' instance attached to '{1}'", typeof(T), objects[i].name);
    ...
    private void Awake() {
      ...
      lock (_lock) {
        if (!instantiated) {
          ...
          Utilities.PrintWarningIfEditor("[Singleton] Deleting extra '{0}' instance attached to '{1}'", typeof(T), name);
